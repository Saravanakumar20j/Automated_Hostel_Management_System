<?php
    ...
    $query="INSERT into wardan(firstName,lastName,gender,block,contactNo,econtact,email,password,gname,grelation,gcontact,address,city,pincode,paddress,pcity,ppincode) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
    $stmt = $mysqli->prepare($query);
    $rc=$stmt->bind_param('ssssiissssississi',$fname,$lname,$gender,$block,$contactno,$emcntno,$emailid,$password,$gurname,$gurrelation,$gurcntno,$caddress,$ccity,$cpincode,$paddress,$pcity,$ppincode);
    $stmt->execute();
        echo"<script>alert('Wardan has been Registered!');</script>";
    }
?>
